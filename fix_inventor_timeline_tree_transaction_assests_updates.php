			...
			if($orgConnect) {
				...
				if($resultRepresentativeParentCompany && $resultRepresentativeParentCompany->num_rows > 0) {
					while($getCompanyRow = $resultRepresentativeParentCompany->fetch_object()) {
						...
						$queryFindAllRFIDs = "SELECT rf_id FROM db_application.documentid WHERE rf_id IN (SELECT rf_id FROM db_uspto.representative_transactions WHERE organisation_id = ".$organisationID." AND representative_id =" .$representativeID.") AND date_format(appno_date, '%Y') >= ".$YEAR;
							...
							if($grandEntered === false){
								...
								if($resultAllRepresentative && $resultAllRepresentative->num_rows > 0) {
									...
									if(count($allRepresentativeIDs) > 0) {
										$queryAllRFIDS = "SELECT rf_id FROM db_application.documentid WHERE rf_id IN (SELECT rf_id FROM db_uspto.representative_transactions WHERE representative_id IN (".implode(',', $allRepresentativeIDs).") GROUP BY rf_id) AND date_format(appno_date, '%Y') >= ".$YEAR ." GROUP BY rf_id";
						...
						/*SecurityIn*/
						$querySecurityIn = 'INSERT IGNORE INTO tree_parties (assignor_and_assignee_id, name, tab_id, organisation_id, representative_id, representative_name) SELECT aaa.assignor_and_assignee_id as assignor_and_assignee_id, CASE WHEN r.representative_name IS NOT NULL THEN r.representative_name ELSE aaa.name END as show_name, "12" as tab, "'.$orgRow->organisation_id.'" as organisation_id, "'.$representativeID.'" as representative_id, "'.$con->real_escape_string($nameRepresentaitve).'" as representative_name FROM assignor as `or` LEFT JOIN assignor_and_assignee as aaa ON aaa.assignor_and_assignee_id = or.assignor_and_assignee_id LEFT JOIN representative as r ON r.representative_id = aaa.representative_id INNER JOIN (SELECT ee.rf_id FROM assignee as `ee` INNER JOIN assignment_conveyance as ass ON ass.rf_id = ee.rf_id INNER JOIN documentid as d ON ass.rf_id = d.rf_id INNER JOIN  assignor_and_assignee as aa ON aa.assignor_and_assignee_id = ee.assignor_and_assignee_id LEFT JOIN representative as r1 ON r1.representative_id = aa.representative_id WHERE ass.convey_ty IN ("security", "restatedsecurity") AND ass.employer_assign = 0 AND (aa.name IN ('.implode(",",$allNames).')) AND ee.rf_id IN ('.implode(',', $rfIDs).') GROUP BY ee.rf_id) as temp ON temp.rf_id = or.rf_id GROUP BY show_name';
						//echo $querySecurityIn."<br/>";
						$con->query($querySecurityIn);
						
						/*ReleaseIn*/
						$queryReleaseIn = 'INSERT IGNORE INTO tree_parties (assignor_and_assignee_id, name, tab_id, organisation_id, representative_id, representative_name) SELECT aaa.assignor_and_assignee_id as assignor_and_assignee_id, CASE WHEN r.representative_name IS NOT NULL THEN r.representative_name ELSE aaa.name END as show_name, "13" as tab, "'.$orgRow->organisation_id.'" as organisation_id, "'.$representativeID.'" as representative_id, "'.$con->real_escape_string($nameRepresentaitve).'" as representative_name FROM assignee as `ee` LEFT JOIN assignor_and_assignee as aaa ON aaa.assignor_and_assignee_id = ee.assignor_and_assignee_id LEFT JOIN representative as r ON r.representative_id = aaa.representative_id INNER JOIN (SELECT or.rf_id FROM assignor as `or` INNER JOIN assignment_conveyance as ass ON ass.rf_id = or.rf_id INNER JOIN documentid as d ON ass.rf_id = d.rf_id INNER JOIN  assignor_and_assignee as aa ON aa.assignor_and_assignee_id = or.assignor_and_assignee_id LEFT JOIN representative as r1 ON r1.representative_id = aa.representative_id WHERE ass.convey_ty IN ("release", "restatedsecurity") AND ass.employer_assign = 0 AND  (aa.name IN ('.implode(",",$allNames).')) AND or.rf_id IN ('.implode(',', $rfIDs).') GROUP BY or.rf_id) as temp ON temp.rf_id = ee.rf_id GROUP BY show_name ';
						//echo $queryReleaseIn."<br/>";
						$con->query($queryReleaseIn);
						
						/*ReleaseOut*/
						$queryReleaseOut = 'INSERT IGNORE INTO tree_parties (assignor_and_assignee_id, name, tab_id, organisation_id, representative_id, representative_name) SELECT aaa.assignor_and_assignee_id as assignor_and_assignee_id, CASE WHEN r.representative_name IS NOT NULL THEN r.representative_name ELSE aaa.name END as show_name, "11" as tab, "'.$orgRow->organisation_id.'" as organisation_id, "'.$representativeID.'" as representative_id, "'.$con->real_escape_string($nameRepresentaitve).'" as representative_name FROM assignor as `or` LEFT JOIN assignor_and_assignee as aaa ON aaa.assignor_and_assignee_id = or.assignor_and_assignee_id LEFT JOIN representative as r ON r.representative_id = aaa.representative_id INNER JOIN (SELECT ee.rf_id FROM assignee as ee INNER JOIN assignment_conveyance as ass ON ass.rf_id = ee.rf_id INNER JOIN documentid as d ON ass.rf_id = d.rf_id INNER JOIN  assignor_and_assignee as aa ON aa.assignor_and_assignee_id = ee.assignor_and_assignee_id LEFT JOIN representative as r1 ON r1.representative_id = aa.representative_id WHERE ass.convey_ty IN ("release", "restatedsecurity") AND ass.employer_assign = 0 AND (aa.name IN ('.implode(",",$allNames).') ) AND ee.rf_id IN ('.implode(',', $rfIDs).') GROUP BY ee.rf_id) as temp ON temp.rf_id = or.rf_id GROUP BY show_name ';
						...
						$querySaleCollectionsQuery = 'INSERT IGNORE INTO tree_parties_collection (assignor_and_assignee_id, rf_id, exec_dt, tab_id, representative_id, organisation_id) SELECT aaa.assignor_and_assignee_id as assignor_and_assignee_id, ee.rf_id,(SELECT exec_dt FROM assignor as ap where ap.rf_id = ee.rf_id GROUP BY ap.rf_id LIMIT 1), "1" as tab, "'.$representativeID.'" as representative_id, "'.$orgRow->organisation_id.'" as organisation_id FROM assignee as ee LEFT JOIN assignor_and_assignee as aaa ON aaa.assignor_and_assignee_id = ee.assignor_and_assignee_id LEFT JOIN representative as r ON r.representative_id = aaa.representative_id INNER JOIN (SELECT or.rf_id FROM assignor as `or` INNER JOIN assignment_conveyance as ac ON ac.rf_id = or.rf_id INNER JOIN documentid as d ON ac.rf_id = d.rf_id INNER JOIN  assignor_and_assignee as aa ON aa.assignor_and_assignee_id = or.assignor_and_assignee_id LEFT JOIN representative as r1 ON r1.representative_id = aa.representative_id WHERE ac.convey_ty IN ("assignment", "partialassignment") AND (aa.name IN ('.implode(",",$allNames).')) AND ac.employer_assign = 0  AND or.rf_id IN ('.implode(',', $rfIDs).') GROUP BY or.rf_id) as temp ON temp.rf_id = ee.rf_id';
						...
						/*License-Out*/
						$queryLicenseOutCollectionsQuery = 'INSERT IGNORE INTO tree_parties_collection (assignor_and_assignee_id, rf_id, exec_dt, tab_id, representative_id, organisation_id) SELECT aaa.assignor_and_assignee_id as assignor_and_assignee_id, ee.rf_id,(SELECT exec_dt FROM assignor as ap where ap.rf_id = ee.rf_id GROUP BY ap.rf_id LIMIT 1), "3" as tab, "'.$representativeID.'" as representative_id, "'.$orgRow->organisation_id.'" as organisation_id FROM assignee as ee LEFT JOIN assignor_and_assignee as aaa ON aaa.assignor_and_assignee_id = ee.assignor_and_assignee_id LEFT JOIN representative as r ON r.representative_id = aaa.representative_id INNER JOIN (SELECT or.rf_id FROM assignor as `or` INNER JOIN assignment_conveyance as ac ON ac.rf_id = or.rf_id INNER JOIN documentid as d ON ac.rf_id = d.rf_id INNER JOIN  assignor_and_assignee as aa ON aa.assignor_and_assignee_id = or.assignor_and_assignee_id LEFT JOIN representative as r1 ON r1.representative_id = aa.representative_id WHERE ac.convey_ty IN ("license", "licenseend", "govern") AND (aa.name IN ('.implode(",",$allNames).')) AND ac.employer_assign = 0  AND or.rf_id IN ('.implode(',', $rfIDs).') GROUP BY or.rf_id) as temp ON temp.rf_id = ee.rf_id ';
						//echo $queryLicenseOutCollectionsQuery."<br/>";
						$con->query($queryLicenseOutCollectionsQuery);
						
						/*SecurityOut*/
						$querySecurityOutCollectionsQuery = 'INSERT IGNORE INTO tree_parties_collection (assignor_and_assignee_id, rf_id, exec_dt, tab_id, representative_id, organisation_id) SELECT aaa.assignor_and_assignee_id as assignor_and_assignee_id, ee.rf_id,(SELECT exec_dt FROM assignor as ap where ap.rf_id = ee.rf_id GROUP BY ap.rf_id LIMIT 1), "4" as tab, "'.$representativeID.'" as representative_id, "'.$orgRow->organisation_id.'" as organisation_id FROM assignee as ee LEFT JOIN assignor_and_assignee as aaa ON aaa.assignor_and_assignee_id = ee.assignor_and_assignee_id LEFT JOIN representative as r ON r.representative_id = aaa.representative_id INNER JOIN (SELECT or.rf_id FROM assignor as `or` INNER JOIN assignment_conveyance as ass ON ass.rf_id = or.rf_id INNER JOIN documentid as d ON ass.rf_id = d.rf_id INNER JOIN  assignor_and_assignee as aa ON aa.assignor_and_assignee_id = or.assignor_and_assignee_id LEFT JOIN representative as r1 ON r1.representative_id = aa.representative_id WHERE ass.convey_ty IN ("security", "restatedsecurity") AND (aa.name IN ('.implode(",",$allNames).')) AND ass.employer_assign = 0 AND or.rf_id IN ('.implode(',', $rfIDs).') GROUP BY or.rf_id) as temp ON temp.rf_id = ee.rf_id ';
						//echo $querySecurityOutCollectionsQuery."<br/>";
						$con->query($querySecurityOutCollectionsQuery);
						
						/*SecurityIn*/
						$querySecurityInCollectionsQuery = 'INSERT IGNORE INTO tree_parties_collection (assignor_and_assignee_id, rf_id, exec_dt, tab_id, representative_id, organisation_id) SELECT aaa.assignor_and_assignee_id as assignor_and_assignee_id, or.rf_id, or.exec_dt, "12" as tab, "'.$representativeID.'" as representative_id, "'.$orgRow->organisation_id.'" as organisation_id FROM assignor as `or` LEFT JOIN assignor_and_assignee as aaa ON aaa.assignor_and_assignee_id = or.assignor_and_assignee_id LEFT JOIN representative as r ON r.representative_id = aaa.representative_id INNER JOIN (SELECT ee.rf_id FROM assignee as `ee` INNER JOIN assignment_conveyance as ass ON ass.rf_id = ee.rf_id INNER JOIN documentid as d ON ass.rf_id = d.rf_id INNER JOIN  assignor_and_assignee as aa ON aa.assignor_and_assignee_id = ee.assignor_and_assignee_id LEFT JOIN representative as r1 ON r1.representative_id = aa.representative_id WHERE ass.convey_ty IN ("security", "restatedsecurity") AND (aa.name IN ('.implode(",",$allNames).')) AND ass.employer_assign = 0 AND ee.rf_id IN ('.implode(',', $rfIDs).') GROUP BY ee.rf_id) as temp ON temp.rf_id = or.rf_id ';
						//echo $querySecurityInCollectionsQuery."<br/>";
						$con->query($querySecurityInCollectionsQuery);
						
						/*ReleaseIn*/
						$queryReleaseInCollectionsQuery = 'INSERT IGNORE INTO tree_parties_collection (assignor_and_assignee_id, rf_id, exec_dt, tab_id, representative_id, organisation_id) SELECT aaa.assignor_and_assignee_id as assignor_and_assignee_id,or.rf_id, or.exec_dt, "11" as tab, "'.$representativeID.'" as representative_id, "'.$orgRow->organisation_id.'" as organisation_id FROM assignor as `or` LEFT JOIN assignor_and_assignee as aaa ON aaa.assignor_and_assignee_id = or.assignor_and_assignee_id LEFT JOIN representative as r ON r.representative_id = aaa.representative_id INNER JOIN (SELECT ee.rf_id FROM assignee as `ee` INNER JOIN assignment_conveyance as ass ON ass.rf_id = ee.rf_id INNER JOIN documentid as d ON ass.rf_id = d.rf_id INNER JOIN  assignor_and_assignee as aa ON aa.assignor_and_assignee_id = ee.assignor_and_assignee_id LEFT JOIN representative as r1 ON r1.representative_id = aa.representative_id WHERE ass.convey_ty IN ("release", "restatedsecurity") AND (aa.name IN ('.implode(",",$allNames).')) AND ass.employer_assign = 0 AND ee.rf_id IN ('.implode(',', $rfIDs).') GROUP BY ee.rf_id) as temp ON temp.rf_id = or.rf_id ';
						//echo $queryReleaseInCollectionsQuery."<br/>";
						$con->query($queryReleaseInCollectionsQuery);
						
						/*ReleaseOut*/
						$queryReleaseOutCollectionsQuery = 'INSERT IGNORE INTO tree_parties_collection (assignor_and_assignee_id, rf_id, exec_dt, tab_id, representative_id, organisation_id) SELECT aaa.assignor_and_assignee_id as assignor_and_assignee_id,ee.rf_id,(SELECT exec_dt FROM assignor as ap where ap.rf_id = ee.rf_id GROUP BY ap.rf_id LIMIT 1) as exec_dt, "13" as tab, "'.$representativeID.'" as representative_id, "'.$orgRow->organisation_id.'" as organisation_id FROM assignee as `ee` LEFT JOIN assignor_and_assignee as aaa ON aaa.assignor_and_assignee_id = ee.assignor_and_assignee_id LEFT JOIN representative as r ON r.representative_id = aaa.representative_id INNER JOIN (SELECT or.rf_id FROM assignor as `or` INNER JOIN assignment_conveyance as ass ON ass.rf_id = or.rf_id INNER JOIN documentid as d ON ass.rf_id = d.rf_id INNER JOIN  assignor_and_assignee as aa ON aa.assignor_and_assignee_id = or.assignor_and_assignee_id LEFT JOIN representative as r1 ON r1.representative_id = aa.representative_id WHERE ass.convey_ty IN ("release", "restatedsecurity") AND (aa.name IN ('.implode(",",$allNames).')) AND ass.employer_assign = 0 AND or.rf_id IN ('.implode(',', $rfIDs).') GROUP BY or.rf_id) as temp ON temp.rf_id = ee.rf_id ';
						//echo $queryReleaseOutCollectionsQuery."<br/>";
						$con->query($queryReleaseOutCollectionsQuery);
						...
						/*CourtOrders*/
						$queryCourtOrderInCollectionsQuery = 'INSERT IGNORE INTO tree_parties_collection (assignor_and_assignee_id, rf_id, exec_dt, tab_id, representative_id, organisation_id) SELECT aaa.assignor_and_assignee_id as assignor_and_assignee_id, ee.rf_id,(SELECT exec_dt FROM assignor as ap where ap.rf_id = ee.rf_id GROUP BY ap.rf_id LIMIT 1), "8" as tab, "'.$representativeID.'" as representative_id, "'.$orgRow->organisation_id.'" as organisation_id FROM assignee as ee LEFT JOIN assignor_and_assignee as aaa ON aaa.assignor_and_assignee_id = ee.assignor_and_assignee_id LEFT JOIN representative as r ON r.representative_id = aaa.representative_id INNER JOIN (SELECT or.rf_id FROM assignor as `or` INNER JOIN assignment_conveyance as ass ON ass.rf_id = or.rf_id INNER JOIN documentid as d ON ass.rf_id = d.rf_id INNER JOIN  assignor_and_assignee as aa ON aa.assignor_and_assignee_id = or.assignor_and_assignee_id LEFT JOIN representative as r1 ON r1.representative_id = aa.representative_id WHERE ass.convey_ty = "courtorder" AND (aa.name IN ('.implode(",",$allNames).')) AND ass.employer_assign = 0 AND or.rf_id IN ('.implode(',', $rfIDs).') GROUP BY or.rf_id) as temp ON temp.rf_id = ee.rf_id ';
						...
						$queryMissingChangeAssignorCollectionsQuery = 'INSERT IGNORE INTO tree_parties_collection (assignor_and_assignee_id, rf_id, exec_dt, tab_id, representative_id, organisation_id) SELECT aaa.assignor_and_assignee_id as assignor_and_assignee_id, ee.rf_id,(SELECT exec_dt FROM assignor as ap where ap.rf_id = ee.rf_id GROUP BY ap.rf_id LIMIT 1), "10" as tab, "'.$representativeID.'" as representative_id, "'.$orgRow->organisation_id.'" as organisation_id FROM assignee as `ee` LEFT JOIN assignor_and_assignee as aaa ON aaa.assignor_and_assignee_id = ee.assignor_and_assignee_id LEFT JOIN representative as r ON r.representative_id = aaa.representative_id INNER JOIN (SELECT or.rf_id FROM assignor as `or` INNER JOIN assignment_conveyance as ass ON ass.rf_id = or.rf_id INNER JOIN documentid as d ON ass.rf_id = d.rf_id INNER JOIN  assignor_and_assignee as aa ON aa.assignor_and_assignee_id = or.assignor_and_assignee_id LEFT JOIN representative as r1 ON r1.representative_id = aa.representative_id WHERE ass.convey_ty = "missing" AND (aa.name IN ('.implode(",",$allNames).')) AND ass.employer_assign = 0 AND or.rf_id IN ('.implode(',', $rfIDs).') GROUP BY or.rf_id) as temp ON temp.rf_id = ee.rf_id ';
						...
						$con->query($queryNameChangeAssignorCollectionsQuery);
							...
							/*SecurityIn*/
							try{
								$querySecurityIn = 'INSERT IGNORE INTO `db_application`.`timeline` (rf_id, reel_no, frame_no, record_dt, organisation_id, representative_id, type, original_name, assignor_and_assignee_id, exec_dt, convey_ty, employer_assign, tab) SELECT or.rf_id, ass.reel_no, ass.frame_no, ass.record_dt, "'.$orgRow->organisation_id.'", "'.$representativeID.'" as representative_id, "Assignor" as type, aaa.name as original_name, aaa.assignor_and_assignee_id, or.exec_dt, acc.convey_ty, acc.employer_assign, "12" as tab FROM assignor as `or` INNER JOIN assignor_and_assignee as aaa ON aaa.assignor_and_assignee_id = or.assignor_and_assignee_id LEFT JOIN representative as r ON r.representative_id = aaa.representative_id INNER JOIN assignment as ass ON ass.rf_id = or.rf_id INNER JOIN assignment_conveyance as acc ON acc.rf_id = ass.rf_id INNER JOIN (SELECT ee.rf_id FROM assignee as `ee` INNER JOIN assignment_conveyance as ac ON ac.rf_id = ee.rf_id INNER JOIN documentid as d ON ac.rf_id = d.rf_id INNER JOIN  assignor_and_assignee as aa ON aa.assignor_and_assignee_id = ee.assignor_and_assignee_id LEFT JOIN representative as r1 ON r1.representative_id = aa.representative_id WHERE ac.convey_ty IN ("security", "restatedsecurity") AND (aa.name IN ('.implode(",",$allNames).')) AND ac.employer_assign = 0  AND ee.rf_id IN ('.implode(',', $rfIDs).') GROUP BY ee.rf_id) as temp ON temp.rf_id = or.rf_id GROUP BY or.rf_id, aaa.assignor_and_assignee_id';
								//echo $querySecurityIn."<br/>";
								$con->query($querySecurityIn);
							}catch(Exception $e){
								print_r($e);
							...
							/*ReleaseIn*/
							try{
								$queryReleaseIn = 'INSERT IGNORE INTO `db_application`.`timeline` (rf_id, reel_no,	frame_no, record_dt, organisation_id, representative_id, type, original_name, assignor_and_assignee_id, exec_dt, convey_ty, employer_assign, tab) SELECT ee.rf_id, ass.reel_no, ass.frame_no, ass.record_dt, "'.$orgRow->organisation_id.'", "'.$representativeID.'" as representative_id, "Assignee" as type, aaa.name as original_name, aaa.assignor_and_assignee_id, (SELECT ap.exec_dt FROM assignor as ap WHERE ap.rf_id = ee.rf_id ORDER BY ap.exec_dt ASC LIMIT 1) as exec_dt, acc.convey_ty, acc.employer_assign, "13" as tab FROM assignee as `ee` INNER JOIN assignor_and_assignee as aaa ON aaa.assignor_and_assignee_id = ee.assignor_and_assignee_id LEFT JOIN representative as r ON r.representative_id = aaa.representative_id INNER JOIN assignment as ass ON ass.rf_id = ee.rf_id	INNER JOIN assignment_conveyance as acc ON acc.rf_id = ass.rf_id INNER JOIN (SELECT or.rf_id FROM assignor as `or` INNER JOIN assignment_conveyance as ac ON ac.rf_id = or.rf_id INNER JOIN documentid as d ON ac.rf_id = d.rf_id INNER JOIN  assignor_and_assignee as aa ON aa.assignor_and_assignee_id = or.assignor_and_assignee_id LEFT JOIN representative as r1 ON r1.representative_id = aa.representative_id WHERE ac.convey_ty IN ("release", "restatedsecurity") AND (aa.name IN ('.implode(",",$allNames).')) AND ac.employer_assign = 0 AND or.rf_id IN ('.implode(',', $rfIDs).') GROUP BY or.rf_id) as temp ON temp.rf_id = ee.rf_id GROUP BY ee.rf_id, aaa.assignor_and_assignee_id';
								//echo $queryReleaseIn."<br/>";
								$con->query($queryReleaseIn);
							}catch(Exception $e){
								print_r($e);
							...
							/*ReleaseOut*/
							try{
								$queryReleaseOut = 'INSERT IGNORE INTO `db_application`.`timeline` (rf_id, reel_no,	frame_no, record_dt, organisation_id, representative_id, type, original_name, assignor_and_assignee_id, exec_dt, convey_ty, employer_assign, tab) SELECT or.rf_id, ass.reel_no, ass.frame_no, ass.record_dt, "'.$orgRow->organisation_id.'", "'.$representativeID.'" as representative_id, "Assignor" as type, aaa.name as original_name, aaa.assignor_and_assignee_id, or.exec_dt, acc.convey_ty, acc.employer_assign, "11" as tab FROM assignor as `or` INNER JOIN assignor_and_assignee as aaa ON aaa.assignor_and_assignee_id = or.assignor_and_assignee_id LEFT JOIN representative as r ON r.representative_id = aaa.representative_id INNER JOIN assignment as ass ON ass.rf_id = or.rf_id	INNER JOIN assignment_conveyance as acc ON acc.rf_id = ass.rf_id INNER JOIN (SELECT ee.rf_id FROM assignee as ee INNER JOIN assignment_conveyance as ac ON ac.rf_id = ee.rf_id INNER JOIN documentid as d ON ac.rf_id = d.rf_id INNER JOIN  assignor_and_assignee as aa ON aa.assignor_and_assignee_id = ee.assignor_and_assignee_id LEFT JOIN representative as r1 ON r1.representative_id = aa.representative_id WHERE ac.convey_ty IN ("release", "restatedsecurity") AND (aa.name IN ('.implode(",",$allNames).')) AND ac.employer_assign = 0 AND ee.rf_id IN ('.implode(',', $rfIDs).') GROUP BY ee.rf_id) as temp ON temp.rf_id = or.rf_id GROUP BY or.rf_id, aaa.assignor_and_assignee_id';
								//echo $queryReleaseOut."<br/>";
								$con->query($queryReleaseOut);
							}catch(Exception $e){
								print_r($e);
