...
if(count($variables) == 2) {
	$year = $variables[1];
	if((int)$year > 0) {
		$queryFindCorrectRFIDs = 'Select assignment.* FROM '.$dbUSPTO.'.assignment WHERE assignment.status = 0 AND date_format(record_dt, "%Y") = "'.$year.'" GROUP BY assignment.rf_id';
