...
if((int)mysqli_errno($con)==0){
	...
	if(isset($_REQUEST['p']) && $_REQUEST['p'] != ''){
		...
		echo $queryJob = "SELECT * FROM jobs as j WHERE j.status = 1 ORDER BY j.id DESC limit 1";
...
function insertPatentDataFromCSVDB($db, $db1, $jobData) {
	...
	$patentResult = $db->query("SELECT status FROM patents where number = '".$patentNumber."'");
	...
	if($scrapePatent === true){
		$queryDocs = "SELECT * FROM documentids_copy WHERE grant_doc_num = '".$patentNumber."'";
				...
				while($doc = $docResult->fetch_object()){
					...
					if($resultAssignment && $resultAssignment->num_rows > 0){
						...
						if($counter == 1){
							...
							if(count($getEPOInventorData) > 0){
								$inventorData['list'] = $getEPOInventorData;
								$inventorData['filling_date'] = $doc->grant_date;
								$inventorData['recorded_date'] = '0000-00-00';
						...
						if($docResult){
							...
							if($noOfDocs > 0){
								...
								while($doc = $docResult->fetch_object()){
									...
									if($resultAssignment && $resultAssignment->num_rows > 0){
										...
										if($counter == 1){
											...
											$getEPOInventorData = runEPOAPI("US".$patent->number);
											if(count($getEPOInventorData) > 0){
												$inventorData['list'] = $getEPOInventorData;
												$inventorData['filling_date'] = $doc->grant_date." 00:00:00";
												$inventorData['recorded_date'] = '0000-00-00 00:00:00';
												if(count($inventorData['list']) > 0) {
													foreach($inventorData['list'] as $inventor) {
														$inventor = getName($patent->number,trim($inventor),$db);
														$inventor = formatText($inventor);
														if(!in_array($inventor,$insertAllNames)) {
															array_push($insertAllNames,$inventor);
															array_push($inventorNameList,$inventor);
														...
														$insertInventorData = array('patent_number'=>$patent->number,'name'=>$inventor,'description'=>'Inventor','execution_date'=>date('Y-m-d h:i:s',strtotime($inventorExecutedDate)),'recorded'=>$inventorRecorded,'type'=>0,'reel_no'=>$assignmentData->reel_no,'frame_no'=>$assignmentData->frame_no,'document_file'=>$fileName,'box_type'=>$inventorBoxID);
										...
										if(count($assignorList) > 0 && count($assigneeList) > 0) {
											...
											if(strpos(strtolower($conveyanceType), "release") === false) {
												...
												foreach($assigneeList as $assignee) {
													...
													$recorded = date('Y-m-d', strtotime($originalRecorded));
													$assigneeName = $assignee['ee_name'];
													$assigneeName = getName($patent->number,trim($assigneeName),$db);
													$assigneeName = formatText($assigneeName);
													$assigneeID = 0;
													$creator = 0;
													
													$queryCheck = "SELECT id,description FROM lead_patent_assignment WHERE type IN (0,1) AND patent_number='".$patent->number."' AND name = '".mysqli_real_escape_string($db,$assigneeName)."' order by id ASC LIMIT 1";
													echo "BOB:".$queryCheck."<br/>";
													$queryResult =  $db->query($queryCheck);
													if($queryResult && $queryResult->num_rows > 0) {
														$checkAssignee = $queryResult->fetch_array(MYSQLI_ASSOC);
													...
													/*If assignee not exist insert it*/
													if($assigneeID === 0) {
														$insertAssigneeData = array('patent_number'=>$patent->number,'name'=>$assigneeName,'description'=>$description,'execution_date'=>$executedDate,'recorded'=>$recorded,'type'=>1,'reel_no'=>$reelNo,'frame_no'=>$frameNo,'document_file'=>$fileName,'assignment_no'=>$counter,'box_type'=>$boxType, 'assignment_type'=>strtolower($conveyanceType));
													...
													if($assigneeID > 0) {	
														...
														foreach($assignorList as $assignor) {	
															...
															if($queryResult && $queryResult->num_rows > 0) {
																...
																/*check assignor exist in 3rd party*/
																$queryCheck = "SELECT id,description FROM lead_patent_assignment WHERE  patent_number='".$patent->number."' AND name = '".mysqli_real_escape_string($db,$assignor)."' AND type = 2 order by id ASC LIMIT 1";
																$queryResult =  $db->query($queryCheck);
																if($queryResult && $queryResult->num_rows > 0) {
																	$checkAssignor = $queryResult->fetch_array(MYSQLI_ASSOC);
																	$assignorID = $checkAssignor['id'];
												...
												foreach($assigneeList as $assignee) {
													...
													/*check Assignee exist*/
													$queryCheck = "SELECT id,description FROM lead_patent_assignment WHERE patent_number='".$patent->number."' AND name = '".mysqli_real_escape_string($db,$assigneeName)."'    AND type IN(0,1) order by id ASC";
													...
													if($queryResult && $queryResult->num_rows > 0) {
														...
														$queryCheck = "SELECT id FROM lead_patent_assignment WHERE patent_number='".$patent->number."' AND name = '".mysqli_real_escape_string($db,$assigneeName)."' AND type = 2 order by id ASC LIMIT 1";
													...
													if($assigneeID == 0) {
														...
														$insertAssigneeData = array('patent_number'=>$patent->number,'name'=>$assigneeName,'description'=>$description,'execution_date'=>$executedDate,'recorded'=>$recorded,'type'=>2,'reel_no'=>$reelNo,'frame_no'=>$frameNo,'document_file'=>$fileName,'assignment_no'=>$counter,'box_type'=>$thirdPartyBoxID, 'assignment_type'=>strtolower($conveyanceType));
								...
								if(count($insertAllHeading)>0){
									...
									/*Update Inventor name if original and modified is same*/
									$query = "Select name from lead_patent_assignment WHERE box_type = 1 AND patent_number = '".$patent->number."'";
									$resultQuery = $db->query($query);
									if($resultQuery && mysqli_num_rows($resultQuery) > 0){
										$getList = array();
										while($rowName = mysqli_fetch_object($resultQuery)){
											array_push($getList, '"'.mysqli_real_escape_string($db,$rowName->name).'"');
										}
										if(count($getList) > 0){
