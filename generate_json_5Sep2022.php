		...
		if($patentNumber != null && $patentNumber != '') {
			try{
				...
				if($resultDocument && $resultDocument->num_rows == 0) {
					...
					$queryDocument  = '';
					if($_REQUEST['f'] == 1) {
						$queryDocument = "SELECT appno_doc_num, appno_date, grant_doc_num, grant_date, 0 AS rf_id  FROM db_patent_application_bibliographic.application_grant WHERE grant_doc_num = '".$patentNumber."' " ;
					} else {
						$assetType = 5;
						$queryDocument = "SELECT appno_doc_num, appno_date, '' AS grant_doc_num, '' AS grant_date, 0 AS rf_id, '' FROM db_patent_grant_bibliographic.application_publication WHERE appno_doc_num = '".$patentNumber."' " ;
					}
					$resultDocument = $con->query($queryDocument) ;
				}
				...
				if($resultDocument && $resultDocument->num_rows > 0) {
					...
					if($applicationNumber != "") {
						...
						$queryBiblioInventor = "SELECT * FROM db_patent_application_bibliographic.inventor WHERE appno_doc_num = '".$grantInventorApp."' GROUP BY name";
						
						$resultBiblioInventor = $con->query($queryBiblioInventor);
						
						if($resultBiblioInventor && $resultBiblioInventor->num_rows == 0) {
							$queryBiblioInventor = "SELECT * FROM db_patent_grant_bibliographic.inventor WHERE appno_doc_num = '".$applicationNumber."' GROUP BY name";
							$resultBiblioInventor = $con->query($queryBiblioInventor);
						}
						
						if($resultBiblioInventor && $resultBiblioInventor->num_rows > 0) {
							while($inventorN = $resultBiblioInventor->fetch_object()) {
								$name = $inventorN->family_name ." ".$inventorN->given_name . " ".$inventorN->middle_name ;
								...
								array_push($biblioInventor, array('name'=>$name, 'family_name'=>$familyName, 'given_name'=>$givenName));
					...
					$allAssignors = array();
					if(count($allRFIDs) > 0) {
						$queryAssignors = "SELECT aa.name, r.representative_name FROM assignor_and_assignee as aa INNER JOIN assignor as a ON a.assignor_and_assignee_id = aa.assignor_and_assignee_id LEFT JOIN representative as r ON r.representative_id = aa.representative_id WHERE a.rf_id IN(".implode(',', $allRFIDs).") GROUP BY a.rf_id, a.assignor_and_assignee_id";
						$resultAllAssignors = $con->query($queryAssignors);
						if($resultAllAssignors && $resultAllAssignors->num_rows > 0){
							while($rowAllAssignor = $resultAllAssignors->fetch_object()){
								$name = $rowAllAssignor->representative_name;
								if($name == null || $name == ''){
									$name = $rowAllAssignor->name;
								}
					...
					if(count($allRFIDs) > 0) {
						for($i = 0; $i < count($documentList); $i++){
							...
							if($conveyanceType != 'addresschg'):
								...
								$queryAssignor = "SELECT a.*,  r.representative_name as normalize_name FROM assignor as a LEFT JOIN assignor_and_assignee as aaa ON aaa.assignor_and_assignee_id = a.assignor_and_assignee_id LEFT JOIN representative as r ON r.representative_id = aaa.representative_id WHERE a.rf_id = ".$doc->rf_id."  GROUP BY a.rf_id, a.assignor_and_assignee_id";
														...
														if(count($biblioInventor) > 0) {
															if(!in_array($name,$inventorNames)) {
																...
																foreach($biblioInventor as $bibInventor) {
																	...
																	if(strtolower($name) == strtolower($bibInventor['name'])) {
																		$cType = 0;
																		$checkBiblio = true;
					...
					if($assetType === 4) {
						if(strpos($patentNumber, '/') === false) {
							$patentNumber = is_numeric($patentNumber) ? number_format($patentNumber) : substr($patentNumber, 0, 2).','.number_format(substr($patentNumber,2));
						}						
					} else {
