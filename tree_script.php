	...
	if((int)$organisationID > 0) {
		...
		if($resultOrganisation && $resultOrganisation->num_rows > 0) {
			...
			if($orgConnect) {
				...
				$queryRepresentative = "SELECT representative_id, representative_name,original_name FROM representative WHERE parent_id = 0";
				...
				if($resultRepresentativeParentCompany && $resultRepresentativeParentCompany->num_rows > 0) {
					$allNames = array();
					while($rowRepresentativeNames = $resultRepresentativeParentCompany->fetch_object()){
						$name = $rowRepresentativeNames->representative_name != null ? $rowRepresentativeNames->representative_name : $rowRepresentativeNames->original_name;
						array_push($allNames, '"'.$con->real_escape_string($name).'"');
					}
					
					$queryRepresentative = "SELECT representative_id FROM `db_uspto`.`representative` WHERE representative_name IN (".implode(',', $allNames).")";
					$resultRepresentative = $con->query($queryRepresentative);
					if($resultRepresentative && $resultRepresentative->num_rows > 0){
						$allRfIDs = array();
						while($rowRepresentative = $resultRepresentative->fetch_object()){
							array_push($allRfIDs, $rowRepresentative->representative_id);
						}
						foreach($allRfIDs as $representativeID) {
							$rfIDs = [];
