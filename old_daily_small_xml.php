...
$con = new mysqli("167.172.195.92","db_user_all","wDv%5tgn0O0kMkMN","db_uspto");
//$con1 = new mysqli("165.232.146.68","db_user_inventor","wDv%5tgn0O0kMkMN","db_inventor");
...
$allConveyText = array();
foreach(glob('/var/www/html/volume1/first/*.xml') as $fileName){
	...
	if($getFileContent) {
		try{
			$getFileContent = '<patent-assignments>'.$getFileContent.'</patent-assignments>';
			$xml = simplexml_load_string(trim($getFileContent));
			if ($xml !== false) {
				$xmlObject = new SimpleXMLElement($getFileContent);	
				$assignmentList = $xmlObject->xpath('patent-assignment');
						...
						*/
						if(count($documentData) > 0) {
							foreach($documentData as $documents){
								$queryDocument = "SELECT rf_id, grant_doc_num, appno_doc_num FROM db_uspto.documentid WHERE rf_id = ".$rfID." AND (grant_doc_num = '".$con->real_escape_string($documents['grant_doc_num'])."' OR appno_doc_num = '".$con->real_escape_string($documents['appno_doc_num'])."') LIMIT 1";
