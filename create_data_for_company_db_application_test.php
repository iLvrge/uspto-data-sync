        ...
        if((int)$organisationID > 0) {      
            ...
            if($result && $result->num_rows > 0) {  
                while($row = $result->fetch_object()) {
                    ...
                    if($orgConnect) {
                        $queryRepresentative = "SELECT representative_id, original_name, parent_id, child, representative_name FROM representative WHERE type = 0";
                        if($representativeName != '') {
                        ...
                        if($resultRepresentative && $resultRepresentative->num_rows > 0) {
                            ...
                            while($representative = $resultRepresentative->fetch_object()){
                                if($representative->representative_name == $representativeName) {
            ...
            if($representativeName != '') {
                $query .=  " AND company_name = '".$con->real_escape_string($representativeName)."' ";
            }

            $result = $con->query($query);
            echo $result->num_rows."<br/>";
            echo "companyIDDDD".$companyIDDDD;
            if($result && $result->num_rows > 0) {
                $startTime = microtime(true);
                $startMainTime = microtime(true);
                if($companyIDDDD > 0) { 
                    exec('php -f /var/www/html/trash/find_release_security.php '.(int)$organisationID).' '.$companyIDDDD;
                } else {
                ...
                while($row = $result->fetch_object()) {
                    ...
                    $endMainTime = microtime(true);
                    $summaryMainTime = ($endMainTime - $startMainTime) * 1000; // milliseconds
                    echo "⏱️ Main Query execution time: " . round($summaryMainTime, 3) . " ms\n";
                    ...
                    if((int)$accountType == 2) {
                        ...
                        if($resultParties) {
                            ...
                            if(count($parties) > 0) {
                                ...
                                $TRANSACTION_YEAR = 2000;
                                $layoutID = 5;
                                echo "COUNT PARTIES: ".count($parties)."<br/>";
                                foreach($parties as $party) {
                                    ...
                                    echo $party->entityName.": NO Of ROWS: ".$queryALLP->num_rows."<br/>"; 
                                    if($queryALLP && $queryALLP->num_rows > 0) {
                                        ...
                                        $queryPartyAssets = "SELECT MAX(appno_doc_num) AS appno_doc_num FROM db_uspto.documentid WHERE rf_id IN (
                                            SELECT rf_id FROM db_new_application.activity_parties_transactions where organisation_id = '".$row->organisation_id."' AND company_id = ".$row->company_id."   AND activity_id IN (5, 12) AND release_rf_id IS NULL  GROUP BY rf_id)
                                            AND date_format(appno_date, '%Y') >= ".$TRANSACTION_YEAR."  GROUP BY appno_doc_num";

                                        $resultParty = $con->query($queryPartyAssets);
                                            echo  "NUM ROWS: ".$resultParty->num_rows."<br/>";
                                        if($resultParty && $resultParty->num_rows > 0) {
                                            $allAssets = array();
                                            while($rowP = $resultParty->fetch_object()){
                                                array_push($allAssets, "'".$rowP->appno_doc_num."'");
                                            }

                                            $queryInsertAssets = "INSERT IGNORE INTO db_new_application.assets_with_bank(appno_doc_num, appno_date, grant_doc_num, grant_date, company_id, organisation_id, rf_id, exec_dt, convey_ty, assignor_id, assignor_name, assignee_id, assignee_name)          
                                            ...
                                            FROM db_uspto.documentid AS d1 
                                                            ...
                                                            WHERE appno_doc_num IN ( ".implode(',', $allAssets).") ";

                                            if(count($allPartyIDs) > 0) {
                                                $queryInsertAssets .= " AND (ass1.assignor_and_assignee_id IN (".implode(',', $allPartyIDs).")
                                                OR aor1.assignor_and_assignee_id IN  (".implode(',', $allPartyIDs).") ) ";
                                            }

                                                             
                                            $queryInsertAssets .= " GROUP BY appno_doc_num) AS max_date ON max_date.appno_doc_num = d .appno_doc_num AND max_date.exec_dt = aor.exec_dt
                                            WHERE d.appno_doc_num IN (".implode(',', $allAssets).") ";

                                            if(count($allPartyIDs) > 0) {
                                                $queryInsertAssets .= " AND (ass.assignor_and_assignee_id IN (".implode(',', $allPartyIDs).")
                                                OR aor.assignor_and_assignee_id IN  (".implode(',', $allPartyIDs).") ) ";
                                            } 

                                            $queryInsertAssets .= " GROUP BY d.appno_doc_num, rac.rf_id, aor.assignor_and_assignee_id, ass.assignor_and_assignee_id ";
                                            echo $queryInsertAssets."<br/>";
                                            $con->query($queryInsertAssets);
                                            echo "QUERY DONE<br/>";
                                        }
                                ...
                                echo "NOW IN LOST Assets";

                                $con->query("DELETE FROM db_new_application.lost_assets WHERE company_id = ".$row->company_id." AND organisation_id = ".$row->organisation_id);

                                
                                $queryAssetsWithBank = "SELECT MAX(appno_doc_num) AS appno_doc_num FROM  db_new_application.assets_with_bank WHERE organisation_id = '".$row->organisation_id."' AND company_id = ".$row->company_id." GROUP BY appno_doc_num";

                                $resultAssetsWithBank = $con->query($queryAssetsWithBank);
                                    echo  "NUM ROWS: ".$resultAssetsWithBank->num_rows."<br/>";
                                if($resultAssetsWithBank && $resultAssetsWithBank->num_rows > 0) {
                                    $allAssets = array();
                                    while($rowA = $resultAssetsWithBank->fetch_object()){
                                        array_push($allAssets, "'".$rowA->appno_doc_num."'");
                                    }
                                
                                    $queryLostAssets = "INSERT IGNORE INTO db_new_application.lost_assets(assignor_and_assignee_id, assignor_id, appno_doc_num, appno_date, grant_doc_num, grant_date, rf_id, original_name, representative_name, company_id, organisation_id)
                                                SELECT assignor_and_assignee_id, assignor_id, appno_doc_num, appno_date, grant_doc_num, grant_date, rf_id, name, representative_name, ".$row->company_id.", ".$row->organisation_id." FROM (
                                                    SELECT ass.assignor_and_assignee_id, tawb.assignor_id, doc.appno_doc_num, doc.appno_date, doc.grant_doc_num, doc.grant_date,  rac.rf_id, aaa.name AS name,
                                                     (SELECT representative_name FROM db_uspto.representative WHERE representative_id = aaa.representative_id) AS representative_name  FROM db_new_application.assets_with_bank as tawb
                                                     INNER JOIN db_uspto.documentid AS doc ON doc.appno_doc_num = tawb.appno_doc_num AND doc.appno_doc_num <> 0  AND doc.appno_doc_num <> '' AND doc.appno_doc_num IN (".implode(',', $allAssets).")
                        ...
                        $dashboardCompanyStartTime = microtime(true);
                        exec('php -f /var/www/html/trash/dashboard_with_company.php "'.$row->company_id.'" "'.$row->organisation_id.'"'); 
                        $dashboardCompanyEndTime = microtime(true);
                        $dashboardExecutionTime = ($dashboardCompanyEndTime - $dashboardCompanyStartTime) * 1000; // milliseconds
                        echo "⏱️ Dashboard Query execution time: " . round($dashboardExecutionTime, 3) . " ms\n";
                    }
                    echo "creating report_represetative_assets_transactions_by_account";
                    echo 'php -f /var/www/html/trash/report_represetative_assets_transactions_by_account_test.php '.$row->organisation_id.' "'.$con->real_escape_string($row->company_name).'"';
                    $reportStartTime = microtime(true);
                    exec('php -f /var/www/html/trash/report_represetative_assets_transactions_by_account_test.php '.$row->organisation_id.' "'.$con->real_escape_string($row->company_name).'"');
                    $reportEndTime = microtime(true);
                    $reportExecutionTime = ($reportEndTime - $reportStartTime) * 1000; // milliseconds
                    echo "⏱️ Report Query execution time: " . round($reportExecutionTime, 3) . " ms\n";
                }    
                echo "Loop finished Summary Org and Assets Family and Logos retrieval";
                echo 'php -f /var/www/html/trash/summary.php "'.(int)$organisationID.'" "'.$representativeName.'" "1"';
                $summaryStartTime = microtime(true);
                exec('php -f /var/www/html/trash/summary.php "'.(int)$organisationID.'" "'.$representativeName.'" "1"');
                $summaryEndTime = microtime(true);
                $summaryExecutionTime = ($summaryEndTime - $summaryStartTime) * 1000; // milliseconds
                echo "⏱️ Summary Query execution time: " . round($summaryExecutionTime, 3) . " ms\n";
                /**
