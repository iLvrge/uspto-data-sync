...
$query = 'SELECT * FROM '.$dbBusiness.'.organisation WHERE org_pass <> "" AND type = 0';
$result = $con->query($query);
if($result && $result->num_rows > 0) {
	while($organisation = $result->fetch_object()) {
		
		try{
			$orgConnect = new mysqli($organisation->org_host,$organisation->org_usr,$organisation->org_pass,$organisation->org_db);
			
			if($orgConnect) {
				$queryRepresentative = "SELECT representative_id, original_name, representative_name FROM representative WHERE type = 0 ";
