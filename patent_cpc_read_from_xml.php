...
foreach(glob('/mnt/volume_sfo2_12/DOWNLOAD/US_Grant_CPC_MCF_XML_2025-05-01/*.xml') as $fileName){
	echo $fileName."<br/>";
			...
			if ($xml !== false) {
				...
				if(count($cpcMasterClassificationRecord)>0){
					...
					foreach($cpcMasterClassificationRecord as $masterClassification){						
						...
						$patentGrantIdentification = $masterClassification->xpath('pat:PatentGrantIdentification');
						
						if($patentGrantIdentification != null) {							
							$ipoOfficeCode = $patentGrantIdentification[0]->xpath('com:IPOfficeCode');
							if($ipoOfficeCode != null) {
								$country = (string)$ipoOfficeCode[0][0];
							}
							$patentNumberText = $patentGrantIdentification[0]->xpath('pat:PatentNumber');
							if($patentNumberText != null){
								$patentNumber = (string)$patentNumberText[0][0];
							}
							$patentCode = $patentGrantIdentification[0]->xpath('com:PatentDocumentKindCode');
							if($patentCode != null) {
								$patentKindCode = (string)$patentCode[0][0];
							}
							$patentDate = $patentGrantIdentification[0]->xpath('pat:GrantDate');
							if($patentDate != null) {
								$patentGrantDate = (string)$patentDate[0][0];
							}
						}						
							...
							if($patMainCPCList != null) {
								foreach($patMainCPCList as $patMainCPC)	{
									...
									if($patCPCClassification != null) {
										...
										$symbolPositionCodeObj = $patCPCClassification[0]->xpath('com:SymbolPositionCode');
										...
										array_push($cpc, array('application_number'=>$applicationNumber,'patent_number'=>$patentNumber,'country'=>$country,'kind_code'=>$patentKindCode,'grant_date'=>$patentGrantDate,'classification_version_date'=>$classificationVersionDate, 'section'=>$section,'class'=>$class,'sub_class'=>$subClass,'main_group'=>$mainGroup,'sub_group'=>$subGroup,'symbol_position_code'=>$symbolPositionCode,'classification_value_code'=>$cpcClassificationValueCode,'type'=>0));
									}
								}
							}
							...
							if($patFurtherCPC != null) {
								...
								foreach($patCPCClassificationList as $patCPCClassification)	{
									...
									if($patCPCClassification != null) {
										...
										array_push($cpc, array('application_number'=>$applicationNumber,'patent_number'=>$patentNumber,'country'=>$country,'kind_code'=>$patentKindCode,'grant_date'=>$patentGrantDate,'classification_version_date'=>$classificationVersionDate, 'section'=>$section,'class'=>$class,'sub_class'=>$subClass,'main_group'=>$mainGroup,'sub_group'=>$subGroup,'symbol_position_code'=>$symbolPositionCode,'classification_value_code'=>$cpcClassificationValueCode,'type'=>1));
									}
								}
							}
						}			
